...
import {getAlgorandGenesisHash, getGenesisHash} from './index';
...
const ALGORAND_GENESIS_HASH = 'wGHE2Pwdvd7S12BL5FaOP20EGYesN73ktiC1qzkkit8=';
...
describe('test jsonrpc client', () => {
  ...
  it("Query algorand's genesisHash via http endpoint", async () => {
    const genesisHash = await getAlgorandGenesisHash(HTTP_ENDPOINT);
    expect(genesisHash).toEqual(ALGORAND_GENESIS_HASH);
  });
});
